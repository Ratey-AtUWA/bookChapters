...
assessments.collected in this class. We don't cover vector-based maps, importing
GIS shapefiles, interpolation of point data, chloropleth maps, and many other
...
afs21ll <- read.csv(paste0(git,"bookChapters/main/afs21ll.csv"), stringsAsFactors = T)
...
st_crs(afs21utm)$input   # check we got the right projection
cat("afs21utm is in",substr(st_crs(afs21utm)$wkt,1,stringr::str_locate(st_crs(afs21utm)$wkt,",")))
```
...
```{r plot-map1-axes, fig.height=5.0, fig.width=6.6, fig.align='center', fig.cap="Map of Ashfield Flats in the UTM projection, message=FALSE, warning=FALSE, generated using the maptiles R package, with OpenStreetMap tiles.", results='hold'}
aftiles <- maptiles::get_tiles(extent, provider = "OpenStreetMap", crop = TRUE, 
                               zoom=17)
...
```{r map-north-scale, fig.height=5, fig.width=6.6, , fig.align='center', fig.cap="Map of Ashfield Flats (UTM), with added North arrow and scale bar from prettymapr, over OpenStreetMap tiles acquired using maptiles.", message=FALSE, warning=FALSE, results='hold',echo=FALSE}
...
```{r add-data, fig.height=5, fig.width=6.6, eval=TRUE, message=FALSE, warning=FALSE, echo=FALSE, fig.cap = "Map of Ashfield Flats (UTM), with added North arrow, scale bar, and user data, over OpenStreetMap tiles acquired using maptiles.", fig.align='center', results='hold'}
par(oma=c(0,0,0,0), lend="square", xpd=TRUE)
plot_tiles(aftiles, adjust=F, axes=TRUE, mar=c(3,3,0.5,0.5)) # use axes = TRUE
...
addscalebar(plotepsg = 32750, label.col = 1, linecol = 1, 
            label.cex = 1.2, htin=0.15, widthhint = 0.15)
clrz <- plasma(15)[6:15]
with(afs21utm, plot(geometry, add=TRUE,  
                    pch = rep(21:25,2)[Group], 
                    bg = clrz[Group]))
legend("bottomright", legend=levels(as.factor(afs21utm$Group)),
       pch = rep(21:25,2), pt.bg = clrz,
       title = "Group", inset = 0.02, ncol = 2)
```
...
```{r map-annot, echo=FALSE, fig.align='center', error=FALSE, fig.cap="Map of Ashfield Flats (UTM), fig.height=5, fig.width=6.6, warning=FALSE, with added North arrow, scale bar, user data, and additional map items, over OpenStreetMap tiles acquired using maptiles.", results='hold'}
# everything we did above in the first line
par(oma=c(0,0,0,0), mar=c(3,3,0.5,0.5), lend="square", xpd=TRUE)
plot_tiles(aftiles, adjust=F, axes=TRUE) # use axes = TRUE
...
```{r map-text-labels, error=FALSE, warning=FALSE, echo=FALSE, fig.height=5, fig.width=6.6, fig.align='center', out.width="70%", fig.cap="Map of Ashfield Flats (UTM), with added North arrow, scale bar, user data, and additional map items plus text labels, over OpenStreetMap tiles acquired using maptiles.", results='hold'}
# everything we did above in the first line
par(oma=c(0,0,0,0), lend="square", xpd=TRUE)
plot_tiles(aftiles, adjust=F, axes=TRUE, mar=c(3,3,0.5,0.5)) # use axes = TRUE
...
```{r map-bubbles, fig.align='center', fig.cap="\\label{bubbles}Bubble map of zinc concentrations in sediment and soil at Ashfield Flats in 2019, over OpenStreetMap tiles acquired using maptiles.", fig.height=5.6, fig.width=7.25, message=FALSE, warning=FALSE, results='hold'}
par(oma=c(0,0,0,0), lend="square", xpd=TRUE)

v0 <- "Zn"

mapdata <- data.frame(st_drop_geometry(afs21utm[,v0]),
                      Easting=st_coordinates(afs21utm)[,1],
                      Northing=st_coordinates(afs21utm)[,2])
...
```{r PctBubb, fig.align='center', fig.cap="Map showing percentile ranges of zinc concentrations in sediment and soil at Ashfield Flats in 2019, over OpenStreetMap tiles acquired using maptiles.", out.width='70%', fig.height=5.5, fig.width=7.25, message=FALSE, warning=FALSE, results='hold'}
...
```{r ggmap1, fig.height=5, fig.width=6.6, fig.align='center', fig.cap="Map of the University of Western Australia campus at Crawley, showing the location of libraries and cafés, plotted over Google maps tiles acquired using the R package ggmap.", out.width='75%', message=FALSE, warning=FALSE, results='hold'}
